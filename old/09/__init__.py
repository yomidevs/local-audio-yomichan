...
FORVO_DB_PATH = get_program_root_path() + "/" + FORVO_DB_NAME
FORVO_DB = sqlite3.connect(FORVO_DB_PATH, check_same_thread=False)
...
class AccentHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def get_forvo_sources(self, term, user):
        audio_sources = []
        users = len(user)
        ...
        with FORVO_DB as db:
            if (users > 0):
                args = [term] + user
                rows = db.execute(
                    f"SELECT speaker,path FROM forvo_audio WHERE expr = ? and speaker IN ({','.join(['?'] * users)}) ORDER BY speaker", (args)).fetchall()
            else:
                rows = db.execute(
                    "SELECT speaker,path FROM forvo_audio WHERE expr = ? ORDER BY speaker", ([term])).fetchall()
            ...
            if (users > 0):
                for u in user:
                    for row in rows:
                        ...
                        if (u == row[0]):
                            audio_sources += [
                                {"name": name, "url": f"http://{HOSTNAME}:{PORT}/{FORVO_PATH}{row[1]}"}]
            else:
                for row in rows:
                    name = row[0]
                    audio_sources += [{"name": name,
    ...
    def get_forvo_audio(self):
        audio_file = get_program_root_path() + \
            f"/{FORVO_MEDIA_DIR}/" + \
            remove_prefix(unquote(self.path), f"/{FORVO_PATH}/")
        if not Path(audio_file).is_file() or not audio_file.endswith(".mp3"):
            self.send_response(400)
            return
        self.send_response(200)
    ...
    def parse_query_components(self):
        ...
        sources = query_components["sources"][0].split(',') if "sources" in query_components else [
            JPOD_SOURCE_PARAM, JPOD_ALTERNATE_SOURCE_PARAM, NHK16_SOURCE_PARAM, FORVO_SOURCE_PARAM]
    ...
    def do_GET(self):
        if self.path.startswith(f"/{JPOD_PATH}/"):
            ...
            self.get_forvo_audio()
            return

        term, reading, sources, user = self.parse_query_components()
        ...
        for source in sources:
            if source == JPOD_SOURCE_PARAM:
                ...
                audio_sources += self.get_forvo_sources(term, user)
